...
**核心概念**: CUDA 是护城河，不只是因为芯片设计得好，也不只是因为软件写得好——是硬件能力、软件生态（编译器、kernel 库、每个主流框架的性能关键路径）、以及两者长期协同打磨这三样一起积累了十几年。这份积累难被替代，恰恰是因为软硬件之间的决策权从来不集中在一个点上，而是分散在编译器、kernel、运行时好几层——挑战者要接住的不是一个开关，是一整条链。
...
## 护城河是什么：硬件、软件、协同设计叠在一起

[Hardware Map](hardware-map.md) 已经说过，CPU、GPU、其他加速器只是"怎么提供算力"这个问题的不同设计取舍，不是谁天生更强。但 GPU 要真正把算力发挥出来，不是硬件自己的事，也不是软件自己的事，而是两者长期咬合在一起的结果：芯片怎么设计，会参考真实工作负载和已有软件生态反馈回来的需求；软件（[Software × Hardware Map](software-hardware-map.md) 已经认识过的 kernel、编译器）怎么优化，也是针对这一代硬件的具体特性去调的。这种"硬件设计和软件优化互相反馈、一起打磨"的**协同设计（Hardware/Software Co-design）**，才是护城河真正的样子——不是"软件比硬件更重要"这么简单的二选一。

CUDA 的护城河，是硬件能力、软件生态、两者协同打磨这三样一起积累了十几年：芯片本身的持续迭代、cuDNN/cuBLAS 这类专门优化过的核心运算库、成熟的编译工具链，以及几乎所有主流 ML 框架的高性能路径都是先为它写、为它调优的。换一块硬件，就算新硬件本身很有竞争力，也要重新走一遍"硬件特性反馈软件优化、软件需求反馈硬件设计"这整个协同过程——这才是真正难替代的地方。
...
- [Software × Hardware Map](software-hardware-map.md) —— Compiler/Kernel/CUDA 最早在这里被认识
- [Hardware Map](hardware-map.md) —— "处理器不止一种"，硬件设计取舍这件事的起点
