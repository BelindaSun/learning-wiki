...
寄存器 / 缓存（书桌上摊开的）        —— 最快，也最小
      ↓
主存 / RAM（书桌旁的书架）          —— 快，中等大小
   （GPU / 加速器旁常见的是 HBM——一种专门为高带宽设计的主存，
    跟普通 RAM 是同一层级，只是造法不同，不是另一层）
      ↓
存储 / 硬盘（图书馆）              —— 最慢，但能放最多
```

这一层层的安排叫**内存层级（Memory Hierarchy）**——不是某一层"更好"，是每一层都在"快"和"大"之间做了不同的取舍，缺哪一层都不行。**HBM 不是夹在 RAM 和硬盘之间的独立一层**，它是"主存"这一层的一种具体实现——普通电脑常用的是 DDR 内存条，AI 芯片旁边常用的是 HBM，两者扮演的都是"计算单元旁边、比硬盘快得多的工作台"这个角色，只是用不同的方式造出更高的带宽。
...
**这两者可以互相独立变化**：HBM 之所以在 AI 硬件里被专门拿出来讲，不是因为它装得下更多（很多时候普通 RAM 装得下的反而更多），而是因为它离计算单元更近、通道更宽，**带宽**远高于普通 RAM——同样是"主存"这一层，HBM 是为带宽做了特殊设计的版本。买 HBM 买的是"搬得快"，不是"装得多"。
...
**这套逻辑在真实 LLM 推理里已经有一个写得很细的案例**：[推理基础设施与 Agent 延迟](../ai-core/inference-infrastructure-and-agent-latency.md) 讲了 Prefill（一次性处理整个输入，矩阵乘矩阵 GEMM，数据复用率高，compute-bound）和 Decode（一次只生成一个 token，矩阵乘向量 GEMV，memory-bound）。Decode 之所以是 memory-bound，主要原因是：每生成一个 token，都要把整套模型权重从内存里重新读一遍，但只用它们做很少的运算（算术强度低）——权重本身没有变大，是"每次都要重读一遍"这件事本身效率低。KV cache 的读取也要占一份带宽，而且会随对话变长而越来越重，但它是在权重读取这个大头之上的**叠加因素**，不是 Decode 变成 memory-bound 的主要原因。这篇讲的是"为什么会有 compute-bound / memory-bound 这两种东西"，那篇讲的是"这两种东西在真实推理系统里长什么样"。
