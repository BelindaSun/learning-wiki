...
| Kernel / Library · 内核 / 库 | 针对某种硬件、某个具体运算（如矩阵乘法）写到极致快的小程序块 | 特技预设包：常用高难动作提前练到极致，用时直接调——CUDA 就是最有名的一套 |
| Runtime · 运行时 | 程序运行期间动态调度资源、分配任务、管理内存的掌控者 | 片场执行导演：拿着剧本和口令，现场指挥算力、内存什么时候上、怎么配合 |

（"kernel"是概念，CUDA 只是其中最有名的实现——先认概念，品牌名随时可以换。Runtime 也是 [Software Map](software-map.md) 里出现过的老词，这里被继续展开。）其中 Runtime 要同时并行拿主意的有四件事，下一节单独展开。
...
| Precision · 精度 / 量化 | 调低数字的表示位数（如 FP16→FP8/INT4），用较小的精度损失换速度和显存 | 画质调节：4K 调到 1080P，多数情况下几乎看不出差别，但快了好几倍 |
...
## 穿线场景：100 个人同时给 ChatGPT 发消息

- **表达层（Workload）**：你的 Prompt 和另外 99 人的请求几乎同时到达服务器。
- **翻译 + 预设层（Compiler & Kernel）**：模型的矩阵计算早已被编译，并用写好的 Kernel（特技预设包，比如 CUDA 上的那些）算得飞快。
- **指挥层（Runtime）**：
  - **Batching**：把这 100 个请求打包成一批（拼车大巴）；
  - **Precision**：用 FP8 精度跑（降画质换速度）；
  - **Memory management**：把模型权重锁在显存最快的位置（道具摆在舞台边）；
  - **Scheduling**：一声令下——"这一批，现在跑！"
- **物理层（Hardware）**：GPU 核心、显存、NVLink 一起爆发出物理计算，生成文字发回给你。
...
- 下次听到"vLLM / TensorRT-LLM 把推理做快了"，知道它们本质是在 Runtime 这一层，把 Batching 和 Memory management 做到极致，而不是换了个更聪明的模型。
