...
> 完全没有 AI 技术背景？从这里开始。这不是课程，是一张**最小地图**——用约 30–45 分钟看完这 8 站，你不会成为专家，但会知道这个 Wiki 里其他文章在讨论什么、彼此怎么连接。看完之后，去 [AI Core](docs/ai-core/index.md)、[心智模型](mental-models.md) 或任何你好奇的地方自由探索就行。
>
...
一个好记的比喻：**Training 像十年苦读备考**——吃进海量的书，反复调整脑子里的几十亿个"参数"，又贵又慢，但一次性提前完成；**Inference 像开卷答题**——凭已经学到的本领，照着你现场给的上下文（Context），一个字一个字把答案写出来，按次发生、相对便宜、实时响应。
...
**一句话答案**：传统 Chatbot 主要是一问一答；Agent 更强调围绕一个目标，自主决定并执行多步行动。区别不只是模型更聪明了，而是"下一步做什么"这件事，系统在运行时有多少自主决定的空间。
...
**一句话答案**：想往下看的话——AI 的每一次回答，本质上都是电流在半导体晶体管里的流动，踩在芯片、算力、内存、互连这层物理地基之上。智能不是凭空长出来的代码。

**最简单的解释**：我们每天读到的一句回答，在物理世界里都消耗了实打实的电和算力——它跑在一颗颗芯片上，靠内存把数据放到手边、靠互连把数据搬来搬去。想看清这层地基长什么样：
...
在这个 Wiki 里，我们采用下面这套实用定义（业界叫法并不完全统一）。一句"是什么"的速查——它们的**关系看上面这张图**，下表只逐个解释，不是从上到下越来越高级：

| 概念 | 本质（严谨一点） | 一句话直觉 |
|---|---|---|
| Tool · 工具 | 模型在运行时可以调用的具体外部能力（API 或代码函数） | 工人的锤子 / 剪刀：单纯的执行能力，发指令它才动 |
| Skill · 技能 | 针对某类任务预先写好的 Prompt + 工具组合 + 操作说明 | 工作手册：教工人"遇到 A 先用锤子、再用剪刀"的经验 |
| Workflow · 工作流 | 路径在运行前就基本由人显式定好的确定性任务链 | 工厂流水线：步骤 1-2-3 提前固定，按既定路线走 |
| Agent · 智能体 | 运行时自己决定下一步、循环"决定 → 行动 → 观察"的系统 | 项目负责人：只给最终目标，自己决定调什么工具、走什么路线 |
| MCP · 统一协议 | 连接外部工具 / 数据源的标准通信协议（Model Context Protocol） | AI 界的 USB：插头标准统一，什么工具都能即插即用 |
| Harness · 运行脚手架 | 围绕模型搭起来的整套运行环境：能看到什么、能用什么、怎么拿反馈、哪里不能越界 | 施工现场的整套配置：脚手架、水电、安全围栏一起在——划边界只是其中一部分 |
