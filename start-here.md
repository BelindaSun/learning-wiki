...
**一句话答案**：不是查答案，是在读完你给的所有文字之后，一个词一个词地"猜"接下来最可能出现什么。
...
**一句话答案**：Chatbot 只会"回答"，Agent 会自己"做事"——这是能力升级，更是主体性的转移。
...
Model（模型）——负责"想"
  ↓ 配上
Tool（工具）——模型能调用的具体能力，比如读文件、查天气
  ↓ 打包成
Skill——把"怎么做某件具体任务"写成可复用的说明书
  ↓ 组合进
Workflow（工作流）——把多个步骤按顺序/并行/条件串起来
  ↓ 由
Agent 自主执行，必要时循环、调整
  ↓ 多个 Agent 协作，构成
...
MCP 是让模型和外部工具"对话"的统一接口标准（类似 USB 统一了设备接口）；Harness 则是给 Agent 定的行为边界——它能用什么工具、能碰什么文件、什么时候必须停下来问人。**这些概念在不同产品里的具体实现会有出入，遇到"到底哪个定义对"的问题时，记住：目前业界没有统一定义，看的是"在这个语境下大概指什么"，不用较真找唯一正确答案。**

**你真正需要记住的 3 件事**：
1. Tool 是能力，Skill 是"怎么用这些能力完成一个具体任务"的说明书，Workflow 是"把多个任务串起来"的流程
2. MCP 是连接工具的标准协议；Harness 是给 Agent 划的行为边界，两者不是一回事
3. 这些词在不同公司产品里定义会有细微差异，属于"实用概念"而不是"统一技术标准"

**常见误解**：「Skill、Tool、Workflow 是三个可以互相替换的说法」——不是，它们是分工不同、可以叠加使用的三层东西。
...
- **Chatbot → Agent**：AI 从"回答问题"走向"完成任务"，不是能力升级，是主体性的转移
